...
BEGIN
	...
	DECLARE @DColumnNames NVARCHAR(MAX);
    ...
    BEGIN
        ...
        SELECT @DColumnNames = STRING_AGG('d.' + QUOTENAME(column_name), ', ') FROM information_schema.columns WHERE table_name = @TableName AND table_schema = @SchemaName;
        ...
        SET @strSQL =   'CREATE TRIGGER [trg_' + @TableName + '_Update] ' +
                        ...
                        'BEGIN ' +
                            ...
                            'SELECT ' + @DColumnNames + ' ' +
                            'FROM deleted d ' +
                            'JOIN inserted i ON d.' + QUOTENAME(@PrimaryKeyColumn) + ' = i.' + QUOTENAME(@PrimaryKeyColumn) + '; ' +
                            ' ' +
                            'UPDATE ' + QUOTENAME(@SchemaName) + '.' + QUOTENAME(@TableName) + ' ' +
                            'SET ' + QUOTENAME(@DateModifiedColumn) + ' = GETDATE() ' +
